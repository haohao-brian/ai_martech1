...
# Find all R files, excluding 99_archived directory
cat("Searching for R files in", scripts_dir, "\n")
r_files <- list.files(scripts_dir, pattern = "\\.R$", recursive = TRUE, full.names = TRUE)
# Filter out files from 99_archived
r_files <- r_files[!grepl("99_archived", r_files)]
cat("Found", length(r_files), "R files (excluding 99_archived directory)\n")
