...
poissonCommentAnalysisServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # 導致 analysis_data() 回 empty df、下游 positive_data filter coefficient 報錯
    ...
    positive_data <- reactive({
      ...
      # #655 fix: Guard against empty data / missing columns
      # 當上游 analysis_data 因為公司無 comment_attribute 資料而回空 df 時,
      # 不可直接 filter(coefficient > 0) — empty df 沒有 coefficient 欄位會報
