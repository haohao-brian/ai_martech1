  ...
  moduleServer(id, function(input, output, session) {
    ...
    platform_id <- reactive({
      tryCatch({
        ...
        # Last-resort: try to infer from any active platform in the cfg, else stop earlier
        stop("platform_id resolution failed: ", e$message)
      })
    })
    ...
    poisson_data <- reactive({
      ...
      result <- tryCatch({
        ...
        # Get platform (#655 fix: 用解析過的 active platform 不寫死 cbz)
        # 注意:時間分析依賴 predictor_type=="time_feature" 的 row;若解析出的
        # platform poisson table 沒有 time features 會回 empty df,UI 顯示 No data
