...
#' Extend RColorBrewer palette beyond its native maxcolors cap
#'
#' Returns a ggplot2 discrete scale that uses the requested brewer palette
#' when n <= maxcolors, and interpolates via colorRampPalette beyond that.
#' Prevents "n too large, allowed maximum for palette ... is 8" warnings +
#' color recycling/collision when fill levels exceed the palette cap. (#703)
#'
#' @param palette Character. RColorBrewer palette name (e.g. "Set2", "Set3").
#' @return A ggplot2 discrete scale layer.
brewer_extended <- function(palette = "Set2") {
  pal_fn <- function(n) {
    max_n <- RColorBrewer::brewer.pal.info[palette, "maxcolors"]
    if (n <= max_n) {
      RColorBrewer::brewer.pal(max(n, 3L), palette)
    } else {
      grDevices::colorRampPalette(RColorBrewer::brewer.pal(max_n, palette))(n)
    }
  }
  ggplot2::discrete_scale("fill", paste0(palette, "_ext"), pal_fn)
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$segment_impact_plot <- renderPlot({
      ...
      ggplot(segment_data_long, aes(x = segment, y = percentage, fill = period)) +
        ...
        brewer_extended("Set2")  # was scale_fill_brewer; extends past n=8 cap (#703)
    })
    ...
    output$cost_analysis_plot <- renderPlot({
      ...
      ggplot(cost_data, aes(x = "", y = amount, fill = category)) +
        ...
        brewer_extended("Set3")  # was scale_fill_brewer; extends past n=12 cap (#703)
    })
  })
}