  ...
  server_fn <- function(input, output, session) {
    moduleServer(id, function(input, output, session) {
      ...
      output$category_repurchase <- renderPlotly({
        ...
        # #381: display per-company Chinese product line name instead of code
        # (e.g. "wsg" → "W Pro/MGS 通用單一儀表"). Look up via df_product_line
        # metadata loaded at app init by load_product_lines(). translate() in
        # ui_terminology.csv is shared across all 5 companies and would
        # cross-contaminate codes; df_product_line is per-company correct.
        cat_df$category_display <- if (exists("df_product_line", inherits = TRUE) &&
                                        is.data.frame(df_product_line) &&
                                        "product_line_name_chinese" %in% names(df_product_line) &&
                                        "product_line_id" %in% names(df_product_line)) {
          pl_lookup <- setNames(df_product_line$product_line_name_chinese,
                                 df_product_line$product_line_id)
          out <- unname(pl_lookup[as.character(cat_df$category)])
          missing <- is.na(out) | !nzchar(out)
          out[missing] <- as.character(cat_df$category)[missing]
          out
        } else {
          sapply(cat_df$category, translate)
        }
