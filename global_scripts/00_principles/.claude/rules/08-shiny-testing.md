...
## 預設選擇規則（CRITICAL — 先看這個）

```
Local dev app(127.0.0.1 / localhost)? → agent-browser (default,no exception)
Live remote URL(Posit Connect / Shinyapps)? → safari-browser (with --url lock)
自動化 CI / 迴歸測試? → shinytest2 + testthat
```

**Local 預設一律 agent-browser**。`safari-browser` 是 **remote URL only narrow scope** — 不要用來看 local app。

### 為什麼

| | agent-browser | safari-browser |
|---|---|---|
| 跑 local Shiny 順暢度 | ✅ headless,零 user 干擾 | ❌ 開系統 Safari,跟 user 正在用的視窗混在一起 |
| 同步看 R log | ✅ `.shiny-debug/shiny.log` 一起追 | ❌ Posit Connect 端 log 看不到 |
| Tab 鎖定保護 | ✅ headless 自帶獨立 context | 🟡 需要 `--url <substr>` lock 防誤切 |

> **反 pattern**:default 用 `safari-browser open http://127.0.0.1:...` 看 local app。會混到 user 開的其他 Safari 分頁(macOS 行為),且 R log 同步不到。**唯一例外**:你刻意要驗證「真實使用者瀏覽器 vs headless Chromium 渲染差異」這種 narrow case — 但這種情境很罕見,大多數 local 驗證不需要。
>
> **Canonical narrow exception case (v1.2)**:`/shiny-adaptive-walk <COMPANY>` skill(in `r-shiny-debugger` plugin v1.1+,實作 MP165 v1.2 Track B adaptive overlay)使用 safari-browser 在 local 跑 discovery walk。理由:user-eye ground truth + visible iteration progress 是 adaptive loop 的核心價值,LLM judge 需要看真實 Safari 渲染才能準確分類 defects。Skill 強制 `--url <substr>` lock,且 mechanical regression 步仍用 agent-browser headless。詳見 MP165 v1.2 Dual-Track Architecture 區段。
...
| **互動式測試**（local default） | /shiny-debug + agent-browser | **新功能探索、視覺驗證、除錯(local app)— 一律用這個** |
| **Live Dashboard 驗證**（remote only） | safari-browser (macOS) | **僅** 已部署到 Posit Connect / Shinyapps 的 live URL smoke test |
...
**結論**:三者互補不衝突,但**有明確 default**:
- `shinytest2` 是 CI 主力(跑本地 app 做 input/output 斷言;automated regression)
- **`agent-browser` 是 local 視覺驗證的唯一 default**(能同時看前端 + R log;不論你是要 click 一個按鈕還是 screenshot 全頁,都用這個)
- `safari-browser` 是 **遠端 live URL 驗證才用**(narrow scope — 已部署環境 only,且必須 `--url` lock)

**鐵律**:local app(`127.0.0.1` / `localhost` URL)**永遠不要**默認用 `safari-browser`。除非你真的在驗證「real Safari renderer」這種特殊情境,且明確說明理由。
...
### ⚠️ AI session 必看:不要用 Claude Code Bash `run_in_background: true` 跑 E2E(#593)

**症狀**:`Rscript -e 'testthat::test_dir(...)'` 透過 Bash tool `run_in_background: true` 起的 subprocess 會 silent 死,log 只剩 setup 訊息 + first test file prefix(例如 `ai-buttons:`),然後 harness 立刻通報「completed exit 0」。

**根因**(as observed 2026-05-07):harness 認為 background command 完成時,SIGTERM/SIGHUP 整個 process group,把長跑的 R subprocess 一起殺。`test_dir` / `test_file` / setup.R / chromote 都沒問題 — bug 在 harness ↔ subprocess 交握。

**正確 pattern**:

```bash
# ✅ Foreground synchronous(timeout 內):harness 同步等到結束
NOT_CRAN=true Rscript -e "testthat::test_dir('scripts/global_scripts/98_test/e2e', reporter='summary')"

# ✅ Shell-level &+ 顯式 pgrep 等候(處理超過 timeout 的場景)
NOT_CRAN=true Rscript -e '...' > /tmp/e2e.log 2>&1 &
until ! pgrep -fl "Rscript.*test_dir.*98_test/e2e" > /dev/null 2>&1; do sleep 2; done
cat /tmp/e2e.log
```
...
```yaml
# ❌ run_in_background: true 在當前 Claude Code Bash hook 下 → subprocess SIGTERM
Bash:
  command: "NOT_CRAN=true Rscript -e 'testthat::test_dir(...)' > /tmp/e2e.log 2>&1"
  run_in_background: true
```

驗證:foreground / shell `&` + `pgrep wait` 都 40 PASS / 3 SKIP / 0 FAIL;`run_in_background: true` 死在 first test file prefix 之後。
