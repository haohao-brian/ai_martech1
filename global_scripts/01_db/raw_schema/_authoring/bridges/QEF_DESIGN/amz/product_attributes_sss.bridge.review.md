# Bridge ensemble review: QEF_DESIGN/amz/product_attributes_sss.bridge.yaml

**Bridge yaml**: `01_db/raw_schema/_authoring/bridges/QEF_DESIGN/amz/product_attributes_sss.bridge.yaml`
...
sss bridge: 87 source columns, 15 rows. Part of 7-bridge batch part 2 — class-level findings inherited from bys/rpl/blb iter-1 ensemble (2026-05-05T13:00:00Z), per-PL spot-check applied here.

## Pre-flight refingerprint (this session)

Fingerprint MATCH against current CSV: `3b6d8bba05ab4c63...` (canonical `infer_column_types_for_fingerprint` helper, no drift since 2026-04-29 generator authoring).

## MP160 enumeration check

source_cols (87) == column_mapping.from_column ∪ ignored_columns: PASS

## Per-PL spot-check (this session)

**Mojibake L-3** detected in cols [商品名稱]: `KleenGuardâ¢ V30 Maverick Eye Protection (49309), with KleenVisionâ¢ Anti-Fog Coating, Clear Lenses, Black Frame (Qty 1)` — UTF-8/CP1252 transcoding artifact, same class as rpl L-1.
...
| S-1 | SUGGESTION | DA cross-bridge | sss structurally consistent with blb pattern (no divergent value_map) |

## Cross-bridge note (DA, inherited)

sss passes structural consistency checks against blb (already CONVERGED at iter 2) and bys/rpl (CONVERGED at iter 1):
