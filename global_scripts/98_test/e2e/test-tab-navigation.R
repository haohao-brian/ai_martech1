...
test_that("all sidebar tabs can be navigated without R errors", {
  # SKIP rationale (#587): all_tabs iterator includes BrandEdge sub-tabs
  # which trigger shinytest2 wait_for_idle abort. Same root cause as
  # test-brandedge.R L1. Defer until #588 (dashboardOverview reactive
  # isolation) lands or until shinytest2 polling pattern is replaced.
  #
  # Migration path (post #588 fix, per #590): the inline iterator below
  # (set_inputs + wait_for_idle + assert_no_r_errors per tab) can be
  # collapsed to `assert_tab_loads_heavy(app, tab)` for plotly-heavy
  # entries (BrandEdge sub-tabs) and `assert_tab_loads_light` for the rest.
  testthat::skip("#587: shinytest2 wait_for_idle vs plotly heavy render (BrandEdge tabs in iteration) — see #588")
