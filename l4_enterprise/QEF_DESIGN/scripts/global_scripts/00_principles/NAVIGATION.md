...
Read:
- `MP102` (v1.3): Bridge yaml is Tier-2 Schema Contract; structured `reviewed_by` + sibling review log
- `MP157` / `MP158`: Two-layer YAML authoring (Layer 1 canonical, Layer 2 bridge)
- `MP159`: Glue Authoring Boundary Immutability — Layer 1 not modified during bridge authoring
- `MP160`: AI Schema/Bridge Authoring Completeness — exhaustive enumeration at write time (sister of MP162)
- `MP161`: Company-Scoped Schema Recognition — universal vs company-scoped classification
- `MP162`: Bridge End-to-End Realization — 5-level staged ship-readiness (L1 yaml → L2 raw INSERT → L3 DRV → L4 dashboard → L5 retention audit) with mandatory L5 completion (sister of MP160)

### ETL/DRV Pipeline Design with Missing Human Input

Read:
- `MP163`: Progressive Completeness — pipeline always-runnable regardless of human-input completeness; gaps materialize as visible sentinels (`'unclassified'` / `'UNKNOWN'`); surface-to-attention via `df_*_coverage_audit` table or derived Gsheet; asymptotic completion without redeploy
- `MP154`: Side Effect Defense — field-level sentinel rule that MP163 escalates to architectural level; at every mapping site use sentinel, not NA
- `MP155`: Minimize Human Input — sister to MP163 on the input axis; MP155 minimizes which fields humans must fill, MP163 ensures pipeline runs while humans are filling them
- `MP162`: Bridge End-to-End Realization — orthogonal axis to MP163; MP162 = time/ship-readiness, MP163 = human-input-completeness; both must be satisfied
