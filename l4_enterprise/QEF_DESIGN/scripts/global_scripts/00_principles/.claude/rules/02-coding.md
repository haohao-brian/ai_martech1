...
| 群組 | 名稱 | 用途 | 範例 | Owner / Source commit |
|------|------|------|------|------|
...
| **D06** | Product Recommendation | 產品推薦（RF + conditional probability）| `D06_01` 產品推薦 | #313 / `9d49655` |

### 新增 DRV 群組 SOP（避免編號撞名,#815）

**建立新 D-group 編號前 SHALL 完整檢查 3 個來源:**

1. **`16_derivations/` 既有 file list**:`ls 16_derivations/fn_D*_core.R | grep "D\d\d_"`
2. **git log all branches**:`git log --oneline --all -- "16_derivations/fn_D0X_*"`
3. **本 rules 表 + Owner 欄**

若新 group 對應 spectra change(`/spectra-discuss` / `/spectra-propose`):
- 此 step **必須**在 propose Step 5 寫入 design.md 作為 explicit decision
- 提交 user explicit review group 編號決策(不可 unattended assumption-mode 自動接續)
- 提交完成後**同步更新本表**加新 row + owner / source commit reference

**反 pattern**(#815 root cause):反射性接續最後一個 group 編號,不查 git log → trap for collision。#313 D06 commit message 含「Exploratory implementation per team discussion (Kyle has not finalized exact formulas)」,但**未**同步更新本表 → 後續 author 看 stale 表看不到 D06 → 接續又 D06 → collision。
