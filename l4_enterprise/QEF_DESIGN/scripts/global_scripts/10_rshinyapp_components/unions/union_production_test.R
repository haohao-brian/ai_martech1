...
# Load AI Assistant module (iframe-embedded external chat bot, URL from app_configs$ai_assistant$url)
source_once("scripts/global_scripts/10_rshinyapp_components/ai_assistant/ai_assistant.R")
...
# Phase A.4 — QEF_DESIGN K-means market segmentation (#804, qef-market-segmentation-redesign)
source_once("scripts/global_scripts/10_rshinyapp_components/position/marketSegmentationKmeans/marketSegmentationKmeans.R")
...
main_app_ui <- bs4DashPage(
                                      ...
                                      bs4SidebarMenuItem(
                                       ...
                                       bs4SidebarMenuItem(
                                        ...
                                        bs4SidebarMenuItem(translate("Market Segmentation and Target Market Analysis"), tabName="positionMS", icon=icon("crosshairs")),
                                         bs4SidebarMenuItem(translate("Market Segmentation (K-means)"), tabName="marketSegmentationKmeans", icon=icon("project-diagram")),
                                       ...
                                       # 6. AI Assistant - 嵌入式 chat bot (Ollama)
                                       bs4SidebarMenuItem(
  ...
  body = bs4DashBody(css_deps,
                     bs4TabItems(
                       ...
                       bs4TabItem(tabName="marketSegmentationKmeans", fluidRow(column(12, bs4Card(title=translate("Market Segmentation (K-means)"), status="info", width=12, solidHeader=TRUE, elevation=3, uiOutput("market_segmentation_kmeans_display"))))),
                       ...
                       # AI Assistant tab - embedded chat bot via iframe
                       bs4TabItem(tabName="aiAssistant", fluidRow(column(12, bs4Card(title=translate("AI Assistant"), status="primary", width=12, solidHeader=TRUE, elevation=3, uiOutput("ai_assistant_display"))))) ) ),
...
server <- function(input, output, session){
  ...
  # Phase A.4 — QEF_DESIGN K-means market segmentation
  market_seg_kmeans_comp <- marketSegmentationKmeansComponent("market_seg_kmeans", app_connection, comp_config, translate)
  ...
  # AI Assistant Component - iframe to external chat bot, URL from app_configs$ai_assistant$url
  ai_assistant_comp <- ai_assistantComponent("ai_assistant", app_connection, comp_config, translate)
  ...
  output$dynamic_filter <- renderUI({
    ...
    switch(sidebar_value,
           ...
           "marketSegmentationKmeans" = market_seg_kmeans_comp$ui$filter,
           ...
           "aiAssistant" = ai_assistant_comp$ui$filter,
           # TagPilot components (Phase 2: DNA analysis)
  ...
  # Phase A.4 — QEF_DESIGN K-means market segmentation
  output$market_segmentation_kmeans_display <- renderUI2(
    current_tab = input$sidebar_menu,
    target_tab = "marketSegmentationKmeans",
    ui_component = market_seg_kmeans_comp$ui$display,
    loading_icon = "project-diagram"
  )
  ...
  # AI Assistant UI output (iframe)
  output$ai_assistant_display <- renderUI2(
    current_tab = input$sidebar_menu,
    target_tab = "aiAssistant",
    ui_component = ai_assistant_comp$ui$display,
    loading_icon = "robot"
  )
  ...
  # Phase A.4 — QEF_DESIGN K-means market segmentation
  market_seg_kmeans_res <- init_on_tab("marketSegmentationKmeans", function() market_seg_kmeans_comp$server(input, output, session))
