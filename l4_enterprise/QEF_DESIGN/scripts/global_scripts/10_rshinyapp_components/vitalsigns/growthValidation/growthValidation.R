    ...
    moduleServer(id, function(input, output, session) {
      ...
      growth_data <- reactive({
        ...
        tryCatch({
          ...
          # #811 fix: filter partial month per D05 is_partial_month flag (#803 convention)
          # 業績成長驗證 latest_row 不應該指向 partial month — 數值會誤導
          if ("is_partial_month" %in% colnames(df)) {
            n_before <- nrow(df)
            df <- df[!df$is_partial_month, ]
            n_dropped <- n_before - nrow(df)
            if (n_dropped > 0) {
              message(sprintf("[growthValidation] Dropped %d partial month(s) per #811 fix", n_dropped))
            }
          }
