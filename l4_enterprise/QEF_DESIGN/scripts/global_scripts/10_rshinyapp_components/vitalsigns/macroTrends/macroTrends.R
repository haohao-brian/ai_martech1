    ...
    moduleServer(id, function(input, output, session) {
      ...
      macro_data_raw <- reactive({
        ...
        tryCatch({
          ...
          # #811 fix: filter out partial month per D05 is_partial_month flag (#803 convention)
          # 客戶反映 2026/3 partial 在趨勢圖上像怪異衰退 → drop partial month to keep MoM/YoY clean
          if ("is_partial_month" %in% colnames(df)) {
            n_before <- nrow(df)
            df <- df[!df$is_partial_month, ]
            n_dropped <- n_before - nrow(df)
            if (n_dropped > 0) {
              message(sprintf("[macroTrends] Dropped %d partial month(s) per is_partial_month flag (#811)",
                              n_dropped))
            }
          }
