...
import_product_profiles <- function(db_connection = raw_data,
  ...
  if (length(results) > 0) {
    tryCatch({
      ...
      # #469: include SKU column so try_read_catalogue + detect_anomalies
      # downstream consumers (fn_resolve_company_product_master.R) can populate
      # System-Sourced ASIN derivation. Pre-fix this helper dropped SKU during
      # union — symmetric bug with the inline ETL union path. Conditional NA
      # for product_profile_<line> tabs that lack SKU column (cross-co safe).
      master_rows <- lapply(names(results), function(pl_id) {
        ...
        out <- data.frame(
          ...
          sku = if ("SKU" %in% names(df)) as.character(df$SKU)
                else if ("sku" %in% names(df)) as.character(df$sku)
                else NA_character_,
