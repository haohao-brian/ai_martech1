...
# --- Test 4: DM_R054 v2.1.1 §6 + F21 — zero-row product line table must stop()
# (#427 Finding #21): if meta_data.duckdb has df_product_line but it's empty,
# reader MUST stop with actionable message identifying the source, rather than
# silently return a 0-row tibble that trips downstream get_active_product_lines.

meta_empty_table <- file.path(tmpdir, "meta_empty_table.duckdb")
ec4 <- DBI::dbConnect(duckdb::duckdb(), meta_empty_table, read_only = FALSE)
DBI::dbExecute(ec4, "CREATE TABLE df_product_line (
  product_line_id TEXT,
  product_line_name_english TEXT,
  product_line_name_chinese TEXT
)")   # table exists but empty
DBI::dbDisconnect(ec4, shutdown = TRUE)

test4 <- tryCatch(
  {
    result <- load_product_lines(
      conn           = sqlite_conn,
      meta_data_path = meta_empty_table,
      csv_path       = missing_csv
    )
    list(raised = FALSE,
         rows = if (is.data.frame(result)) nrow(result) else NA_integer_)
  },
  error = function(e) list(raised = TRUE, msg = conditionMessage(e))
)

results$empty_table_must_stop <- expect(
  "zero-row df_product_line in meta_data.duckdb → stop() (F21 guard)",
  isTRUE(test4$raised),
  if (isTRUE(test4$raised)) substr(test4$msg, 1, 200) else
    paste("no stop raised, returned rows =", test4$rows)
)
results$empty_table_mentions_source <- expect(
  "stop() message identifies source (meta_data.duckdb / dbConnectAppData)",
  isTRUE(test4$raised) &&
    (grepl("meta_data", test4$msg, ignore.case = TRUE) ||
     grepl("dbconnectappdata", test4$msg, ignore.case = TRUE) ||
     grepl("canonical", test4$msg, ignore.case = TRUE)),
  substr(test4$msg %||% "", 1, 200)
)
