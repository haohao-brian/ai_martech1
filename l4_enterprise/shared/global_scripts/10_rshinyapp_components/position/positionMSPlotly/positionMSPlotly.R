...
get_language_config <- function() {
  ...
  if (exists("app_configs") && !is.null(app_configs$language)) {
    # Defensive scalar normalization: `app_configs$language` may be a
    # character vector len > 1 (multi-locale config / merged inheritance).
    # `grepl()` on vector returns vector → `if (vector)` aborts in R >= 4.2
    # with "the condition has length > 1", causing ai_naming_task to fail
    # and falling back to default segment names. Take [1] to preserve
    # if-else priority order (zh_TW first → zh_CN → en). (#704)
    lang <- app_configs$language[1]
...
positionMSPlotlyServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # ------------ CSA Diagnostic (#734 MP163 empty-state) -----------
    # Mirror the NA-threshold + numeric-col filter logic to capture WHY
    # perform_csa_analysis returned NULL — so the empty plot/table state
    # can surface a specific actionable message instead of generic "No data".
    csa_diagnostic <- reactive({
      # IC_P002 / Codex-P2-B: mirror csa_result()'s active-tab gate so that
      # inactive BrandEdge MS tabs don't trigger position_data() fetch on
      # PL change elsewhere. Consumers (csa_plot / cluster_table) already
      # null-safe on diag via `!is.null(diag) && diag$status %in% ...`.
      if (!is_component_active()) return(NULL)

      data <- position_data()
      if (is.null(data) || nrow(data) == 0) return(NULL)

      key_cols <- c("product_id", "brand", "product_line_id", "platform_id")
      exclude_vars <- c("product_line_id", "platform_id", "revenue",
                        "rating", "sales", "Revenue", "Rating")
      ...
      total_numeric <- ncol(df_wo)
      if (total_numeric == 0) {
        return(list(status = "no_numeric_cols", good_cols = 0,
                    total_numeric = 0, threshold = input$na_threshold))
      }

      na_props <- sapply(df_wo, function(x) mean(is.na(x)))
      good_count <- sum(na_props <= input$na_threshold)

      status <- if (good_count == 0) "all_filtered_out"
                else if (good_count < 3) "too_few_variables"
                else "ok"

      list(status = status, good_cols = good_count,
           total_numeric = total_numeric, threshold = input$na_threshold)
    })
    ...
    output$csa_plot <- renderPlotly({
      ...
      if (is.null(data) || nrow(data) == 0) {
        ...
        # #734: when reason is "too few variables passed NA threshold" or MDS
        #        degenerated, surface the actionable cause instead of generic "No data".
        ...
        empty_title <- if (is.null(prod_line_check) || is.na(prod_line_check) ||
          ...
          translate("Please select a specific product line to view brand position analysis")
                   ...
                   diag$status %in% c("too_few_variables", "all_filtered_out")) {
          sprintf(
            translate("MS analysis needs at least 3 attribute variables passing the data quality threshold; only %d of %d numeric attributes for product line '%s' qualify (current NA threshold: %.0f%%). Try raising the 'Data Quality Threshold' slider, or this product line may have uneven review coverage."),
            diag$good_cols, diag$total_numeric, prod_line_check,
            diag$threshold * 100
          )
        } else {
          translate("No data available for competitive analysis")
        }
    ...
    output$cluster_table <- renderDT({
      ...
      if (is.null(analysis) || nrow(analysis) == 0) {
        ...
        # #734: surface "too few variables" / "all filtered out" cause when applicable
        prod_line_check <- product_line_id()
        diag <- csa_diagnostic()
        ...
        msg <- if (is.null(prod_line_check) || is.na(prod_line_check) ||
          ...
          translate("No cluster analysis available")
        }
      ...
      if (component_status() == "ready" && !ms_analysis_generated()) {
        ...
        later::later(function() {
          tryCatch({
            ...
            if (is.null(analysis) || nrow(analysis) == 0) {
              ms_ai_analysis_result(translate("No cluster analysis data available for AI report."))
              return()
            }
