...
positionStrategyServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    key_factors <- reactive({
      ...
      # Note: keep Ideal (used as reference at line below); drop Rating/Revenue/Importance
      # Importance added per #562 — sister of #404 F2 fix in positionKFE.R:69
      df_analysis <- data %>%
      ...
      # Extract ideal point vector. Single-bracket on data.frame/tibble returns
      # a data.frame (a list), which as.numeric() cannot coerce — use unlist()
      # to flatten the row to a named numeric vector first. Fixes #586.
    ...
    observeEvent(input$generate_analysis, {
      ...
      if (is.null(data) || nrow(data) == 0) {
        # #600: context-aware notification
      ...
      if (is.null(selected_product_id) || selected_product_id == "") {
        showNotification(translate("Please select a product first"), type = "warning")
        return()
      }

      if (is.null(gpt_key)) {
        showNotification(translate("OpenAI API key not configured. AI analysis features are disabled."), type = "error")
        return()
      }
      ...
      withProgress(message = "Generating strategy analysis for selected product...", value = 0, {
        ...
        if (is.null(current_strategy)) {
          showNotification(translate("Please select a product to see strategy analysis"), type = "warning")
          return()
        }
        ...
        # Wrap in tryCatch to surface errors instead of silent fail (#806)
        txt <- tryCatch({
          chat_api(
          ...
          message("[positionStrategy] chat_api error: ", err_msg)
          showNotification(
            translate(paste0("AI 分析失敗:", err_msg, "(請查 R console 詳細 log)")),
            type = "error",
            duration = 10
          )
          NULL
        ...
        # Defensive: chat_api may return NULL on internal tryCatch silent-swallow (#806)
        if (is.null(txt) || (is.character(txt) && nchar(trimws(txt)) == 0)) {
          message("[positionStrategy] chat_api returned NULL/empty — surfacing to user")
          showNotification(
            translate("AI 分析無回應(可能原因:OPENAI_API_KEY 未設、model 不可用、network timeout)。請查 deploy log 或本地 R console。"),
            type = "error",
            duration = 10
          )
          incProgress(1.0, detail = "Failed")
          return()
        }
    ...
    output$strategy_plot <- renderPlotly({
      ...
      if (is.null(result)) {
        ...
        return(plot_ly() %>%
          add_annotations(
            text = translate("Please select a product to view strategy analysis"),
            x = 0.5, y = 0.5,
