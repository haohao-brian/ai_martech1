...
chat_api <- function(messages,
  ...
  if (!use_chat_api) {
    ...
    # gpt-5.5+ adopts the reasoning-model API contract:
    #   - param renamed `max_tokens` → `max_completion_tokens`
    #   - `temperature` only accepts default (1); 0.3 rejected
    # Treat ^gpt-5\.5 alongside o1/o3 family (same restrictions).
    if (grepl("^o3", model)) {
      body$max_completion_tokens <- 4000
    } else if (grepl("^o1", model)) {
      body$max_completion_tokens <- 2000
    } else if (grepl("^gpt-5\\.5", model)) {
      body$max_completion_tokens <- 1024
    } else {
      # Traditional models (gpt-4, gpt-3.5, gpt-5.4 and earlier)
