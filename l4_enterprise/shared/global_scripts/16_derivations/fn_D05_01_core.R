...
run_D05_01 <- function(platform_id, config = NULL) {
  ...
  tryCatch({
    ...
    # --- Identify order_id column (platform-agnostic, post-glue canonical) ---
    order_id_col <- intersect(
      c("amz_amazon_order_id", "amazon_order_id", "order_id", "order_number"),
      names(sales_raw)
    )
    ...
    aggregate_monthly <- function(df_subset, pl_label) {
      ...
      # #803 fix + #811 month-end bug fix: 加 is_partial_month column 標示「未完整月」
      # 客戶反映:若 2026/3 還沒到月底,MoM(2026/3 partial)看起來像怪異衰退。
      # 此處不過濾,加 column 讓 downstream UI 自行決定要不要畫 partial month。
      # Convention: month M is partial if Sys.Date() falls in M and today < last_day_of_M - 3 days lag buffer
      #
      # #811 fix: 原 `as.Date(format(today + 31L, "%Y-%m-01"))` 在 31 號月份(1/3/5/7/8/10/12)
      # 會跳到下下個月(e.g. today=2026-03-31 → today+31=2026-05-01 → next_month_first=2026-05-01
      # → last_day_current=2026-04-30 但 today 在 3 月,應該是 2026-03-31)。
      # 改用 seq() by month → robust across month-end edge cases (no bug for 30-day/31-day months
      # 也對 Feb 28/29 days 正確)。
      today <- Sys.Date()
      today_ym <- format(today, "%Y-%m")
      this_month_first <- as.Date(format(today, "%Y-%m-01"))
      next_month_first <- seq(this_month_first, by = "month", length.out = 2L)[2L]
      last_day_current <- next_month_first - 1L
      # safe lag: data is considered complete only if today is within last 3 days of month OR later
      is_today_within_complete_window <- as.integer(last_day_current - today) <= 3L

      monthly_df$is_partial_month <- vapply(
        monthly_df$year_month,
        function(ym) {
          if (ym == today_ym && !is_today_within_complete_window) TRUE else FALSE
        },
        logical(1)
      )
