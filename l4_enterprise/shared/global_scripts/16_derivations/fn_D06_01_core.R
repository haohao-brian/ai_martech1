...
run_D06_01 <- function(platform_id, config = NULL) {
  ...
  tryCatch({
    ...
    # Post-glue canonical: standardized table has 'product_id' + 'payment_time'.
    # Legacy chain had 'asin' + 'purchase_date'. Read whichever exists, rename
    # to D06_01's internal variable names (asin / purchase_date) to keep
    # algorithm code unchanged.
    sales_data <- dplyr::tbl(transformed_data, required_table) |>
      dplyr::select(dplyr::any_of(c(
        "customer_id", "asin", "product_id", "purchase_date", "payment_time", "product_line_id"
      ))) |>
      dplyr::collect()
    if (!"asin" %in% names(sales_data) && "product_id" %in% names(sales_data)) {
      sales_data$asin <- sales_data$product_id
      sales_data$product_id <- NULL
    }
    if (!"purchase_date" %in% names(sales_data) && "payment_time" %in% names(sales_data)) {
      sales_data$purchase_date <- sales_data$payment_time
      sales_data$payment_time <- NULL
    }
