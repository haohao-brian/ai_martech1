...
fn_generate_display_name <- function(
  ...
  # ==========================================================================
  # Pre-lookup: Strip `_rating$` architectural suffix (#756 / #733)
  # ==========================================================================
  # D04_02.R LEFT-joins by_asin per-attribute rating cols with a `_rating`
  # suffix (Plan sub-decision D-B avoids dplyr `.x/.y` collision with ts_data's
  # own `rating` column). The suffix is a DB-layer naming detail; display
  # layer should hide it so users see "光學清晰" not "光學清晰_rating".
  #
  # The classifier (fn_classify_predictor_type Stage 2.5, #745) still sees the
  # original suffixed name at DB layer — only the display lookup uses the
  # base name. nchar guard prevents stripping standalone `_rating` to "" so
  # the `^(customer_ratings|rating)` prefix handler below still catches the
  # `rating` overall-rating column.
  if (grepl("_rating$", predictor) && nchar(predictor) > nchar("_rating")) {
    predictor <- sub("_rating$", "", predictor)
  }
