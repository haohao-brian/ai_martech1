...
## Related: Predictor Classification Metadata (2026-05-15, refs #716 / #724)

Change `metadata-driven-predictor-type-classification` established a metadata-driven pattern for DRV `predictor_type` classification. Key artifacts:

- **Schema authoring (R-based)**: `r_definitions/SCHEMA_006_predictor_classification.R` — defines `df_predictor_classification` table living in `meta_data.duckdb` (per DM_R054 v2, NOT `app_data.duckdb`). Layer 1 universal (5 columns: `platform / column_name / predictor_type / source / added_at`; composite PK `(platform, column_name)`; CHECK constraints).
- **Bridge yaml as metadata source**: `04_utils/fn_populate_predictor_classification_from_bridges.R` scans `bridges/<COMPANY>/<platform>/*.bridge.yaml` `field_extractors` and derives `predictor_type` from canonical name **semantic prefix**:
  - `rating_*` / `review_count` / `review_rating` → `comment_attribute`
  - `feature_*` / `protection_*` / `dimension_*` / `color_*` / `material_*` / `accessory_*` / `activity_*` / `scene_*` / `demographic_*` / `user_*` / `cleaning_*` → `product_attribute`
  - `brand` / `sku` / `asin` / `url` / `product_name` → `structural`
- **Producer ETL**: `update_scripts/ETL/all/all_ETL_meta_init_0IM.R` hooks the populate-from-bridges helper after SCHEMA_006 table init. Idempotent via DELETE+INSERT per platform.
- **Canonical reader**: `04_utils/fn_load_predictor_classification.R` provides `load_predictor_classification(platform, column_name, con)` — the only API DRV / Shiny consumers should use to read this table.
- **DRV classifier with metadata dispatch**: `04_utils/fn_classify_predictor_type.R` implements 5-stage dispatch (time → structural → metadata-lookup → English keyword fallback → default `product_attribute`). Sourced by `update_scripts/DRV/D04/D04_02.R`.

**Cross-references**:
- MP064 (ETL-Derivation Separation): predictor classification metadata lives in meta_data.duckdb (7th layer parallel to 6-layer ETL flow per DM_R054 v2)
- MP159 (Glue Authoring Boundary Immutability): bridge yaml `field_extractors` semantic-prefix IS the metadata source (Layer 1 contract); DRV consumes through derived `df_predictor_classification` (Layer 2 materialization)
- DM_R054 v2 (Metadata Storage Policy): `df_predictor_classification` MUST live in `meta_data.duckdb`
- DM_R066 (DRV Platform Agnosticism): `classify_predictor_type` accepts `platform` parameter (no hardcoded platform literal)
