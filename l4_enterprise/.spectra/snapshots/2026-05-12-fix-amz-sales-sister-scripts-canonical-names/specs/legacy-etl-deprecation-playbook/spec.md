...
1. **Pre-migration verify**: Confirm bridge yaml at `bridges/<COMPANY>/<platform>/<datatype>.bridge.yaml` exists AND uses `field_extractors` shape per the schema-driven redesign. When a sibling `.bridge.v2.yaml` exists (P2 migration tool output), the swap to v2 form is part of this step's atomic cutover commit, not a prerequisite that ran earlier
...
6. **DRV consumer transparency check**: Capture only the transformed-layer baseline (`df_<platform>_<datatype>___transformed`) as pre/post snapshot anchor. DRV consumer outputs (D01_xx / D05_xx / D06_xx etc) are automatically verified by the transformed-layer diff because they read only from the transformed table. Rowcount, schema, and key business metrics SHALL match within 0.1% tolerance
...
The entire 7-step sequence SHALL land as one atomic cutover commit on a dedicated feature branch. Intermediate progress MAY accumulate on the feature branch using local commits but SHALL be squashed before merge to main. Partial landing of subsets (e.g., yaml swap merged without 1ST/2TR trim) is FORBIDDEN because it leaves the pipeline in a broken intermediate state where the raw layer has canonical names but downstream R scripts still expect source-original names.

Bridge yamls produced by `migrate_bridge_to_field_extractors.R` migration tool inherit their `reviewed_by` block from the source legacy bridge. The migration is mechanical transformation, not new authorship, so the legacy bridge's review state carries forward unchanged. Per-datatype child changes SHALL preserve this `reviewed_by` block when performing the v2 yaml swap and SHALL NOT trigger a fresh `/glue-bridge` ensemble review for migration-converted bridges. New authoring of brand-new bridges (not migration-converted) still requires the full ensemble review per MP102 v1.3.

If the datatype has sister ETL scripts beyond the standard 0IM/1ST/2TR triple (such as `<platform>_ETL_<datatype>_2TS.R` or `<platform>_ETL_<datatype>_time_series_2TR.R`), the migration SHALL include a regression check for each sister script after the transformed-layer diff passes. Sister scripts that consume `df_<platform>_<datatype>___transformed` are automatically guarded by the transformed-layer diff for column-name correctness but SHALL still execute end-to-end at least once post-migration to confirm no hardcoded source-name dependencies were missed.
...
- **AND** the rowcount of `df_amz_sales___transformed` differs by more than 0.1% between pre and post
- **THEN** the migration SHALL revert all commits on the feature branch
...
#### Scenario: Sister script regression on hardcoded legacy column name

- **WHEN** Step 6 transformed-layer diff passes for amz/sales pilot
- **AND** subsequent sister-script regression check on `amz_ETL_sales_2TS.R` fails because the script has a hardcoded reference to `amazon_order_id` (source-original name)
- **THEN** the pilot SHALL update the sister script in the same atomic cutover commit to use canonical `order_id` instead
- **AND** the update SHALL preserve all behavior beyond the column name change

#### Scenario: Migration-converted bridge reviewed_by preservation

- **WHEN** the v2 yaml swap in Step 1 introduces `sales.bridge.yaml` content from `sales.bridge.v2.yaml`
- **AND** the v2 yaml was produced by `migrate_bridge_to_field_extractors.R` from the prior legacy form
- **THEN** the `reviewed_by` block in the swapped yaml SHALL match the legacy bridge's reviewed_by block exactly
- **AND** no fresh /glue-bridge ensemble review SHALL be triggered
- **AND** if the migration tool failed to copy `reviewed_by` over (tool bug), the author SHALL manually paste the legacy reviewed_by into the v2 form before commit

##### Example: Pre/post baseline snapshot content for amz/sales

| Metric | Pre-migration value | Post-migration acceptance |
| --- | --- | --- |
| `nrow(df_amz_sales___transformed)` | 281784 (captured 2026-05-11) | within ±0.1% of pre value (281,500 - 282,068) |
| `n_distinct(order_id)` | (captured at pre-migration) | within ±0.1% |
| `sum(total_amount, na.rm=TRUE)` for year=2024 | (captured at pre-migration) | within ±0.1% |
| Schema column types | 45 columns, legacy schema (amazon_order_id etc) | post-migration: canonical schema (order_id etc), all 45 columns mapped per bridge field_extractors |

##### Example: Atomic cutover commit file inventory

A compliant atomic cutover commit for amz/sales SHALL contain all of the following file changes (any missing piece is invariant violation, MUST NOT merge):

| File change | Action |
| --- | --- |
| `shared/global_scripts/01_db/raw_schema/_authoring/bridges/QEF_DESIGN/amz/sales.bridge.yaml` | content overwrite (legacy → v2) |
| `shared/global_scripts/01_db/raw_schema/_authoring/bridges/QEF_DESIGN/amz/sales.bridge.v2.yaml` | git rm (content merged into sales.bridge.yaml) |
| `shared/update_scripts/ETL/amz/amz_ETL_sales_1ST.R` | trimmed ~50% (schema-mapping removed) |
| `shared/update_scripts/ETL/amz/amz_ETL_sales_2TR.R` | trimmed ~70% (schema-mapping removed) |
| `shared/update_scripts/ETL/amz/amz_ETL_sales_0IM.R` | git mv to 99_archived/legacy_etl_pre_glue/ |
| `shared/update_scripts/99_archived/legacy_etl_pre_glue/README.md` | new entry for amz_ETL_sales_0IM.R |
| Post-migration snapshot RDS | new file in changelog/reports/ |
| Diff report markdown | new file in changelog/reports/ |
| IC_P002 cross-co dry-run report | new file in changelog/reports/ |


<!-- @trace
source: amz-sales-pilot-execution
updated: 2026-05-12
code:
