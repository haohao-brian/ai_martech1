...
poissonFeatureAnalysisServer <- function(id, app_data_connection = NULL, config = NULL,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # 過去 default "cbz" 對 QEF (amz-only) 不對;統一走 resolve_active_platform
    ...
    analysis_data <- reactive({
      ...
      tryCatch({
        ...
        data <- data %>%
          ...
          # #762 / Codex F1: strip architectural `_rating` suffix in the fallback
          # branch so a stale/missing display_name never re-leaks `<name>_rating`
          mutate(display_name_safe = dplyr::coalesce(display_name, sub("_rating$", "", predictor)))
    ...
    # #655 fix: empty-df helper preserves column shape for downstream pipes
    .empty_positive_data <- function() {
      data.frame(
        coefficient = double(0),
        incidence_rate_ratio = double(0),
        p_value = double(0),
    ...
    positive_data <- reactive({
      ...
      # Check if data is empty (error occurred in analysis_data OR no comment_attribute rows)
      if (is.null(data) || nrow(data) == 0 || ncol(data) == 0) {
        message("DEBUG: analysis_data() returned empty dataframe - check error logs above")
        return(.empty_positive_data())
      }
      ...
      if ("coefficient" %in% names(data)) {
        ...
        # Path 3: Neither column exists - return empty (with shape per #655 fix)
    ...
    output$analysis_table <- DT::renderDT({
      ...
      if ("coefficient" %in% names(data)) {
        ...
        table_data <- data %>%
          ...
          # #762: display_name_safe is the sole user-facing name column (raw predictor removed)
          dplyr::select(
            display_name_safe,
            track_multiplier,
            marginal_effect_pct,
        ...
        # #762: display_name_safe is the sole user-facing name column
        colnames(table_data) <- sapply(c(
          "Attribute Name", "Track Multiplier", "Marginal Effect%",
    ...
    output$download_full <- downloadHandler(
      ...
      content = function(file) {
        ...
        table_data <- data %>%
          ...
          # #762: export display_name_safe only (raw predictor removed)
          dplyr::select(
            display_name_safe,
            track_multiplier,
            marginal_effect_pct,
        ...
        # 設定中文欄位名稱
        # #762: user-friendly column names only
        colnames(table_data) <- sapply(c(
          "Attribute Name", "Track Multiplier", "Marginal Effect%",
          "Coefficient", "Standard Error", "95% Confidence Interval",
          "Sample Size", "P-Value", "Significance", "Business Meaning"
        ...
        if (nrow(positive_vars) == 0) {
          ...
          if ("coefficient" %in% names(positive_vars)) {
            product_dev_summary <- positive_vars %>%
              mutate(
                var_description = paste0(
                  display_name_safe, ": 係數=", round(coefficient, 3),
            ...
            product_dev_summary <- positive_vars %>%
              mutate(
                var_description = paste0(
                  display_name_safe, ": 發生率比=", round(incidence_rate_ratio, 3),
                  ", 賽道倍數=", track_multiplier,
                  ", 邊際效應=", marginal_effect_pct, "%"
                )
              ) %>%
