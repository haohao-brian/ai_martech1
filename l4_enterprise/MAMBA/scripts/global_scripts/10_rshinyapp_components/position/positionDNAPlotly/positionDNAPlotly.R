...
transform_position_to_dna_format <- function(data, exclude_vars = NULL) {
  ...
  # Refs #693: drop NA-score rows during pivot. Without this, the long-format
  # `dna_data` contains rows for attributes that have zero scores across all
  # products in the current product_line (e.g. 偏光效果佳 / 偏光功能 in hsg PL
  # are columns in df_position but have NA for ALL 54 hsg products). Those
  # attributes then leak into:
  #   (1) attribute dropdown choices (built from levels(data$attribute)) —
  #       user sees & selects them expecting data
  #   (2) filtered_dna_data filter pass-through with NA scores
  #   (3) plotly add_trace with NA y → no markers drawn at those x positions →
  #       x-axis ticks for those attributes silently vanish from plot
  # Net effect: confusing UX where multi-select shows only 1 of N selected attrs.
  # values_drop_na = TRUE drops these rows at source — attribute factor levels
  # only contain attributes with ≥1 non-NA score → dropdown reflects reality.
  # #788 fix: cast to factor BEFORE arrange so row order matches factor levels.
  # Background: `dplyr::arrange()` on a character column uses **radix / byte-
  # order sort** (via `vctrs::vec_proxy_order`); `as.factor()` sets levels via
  # base R `sort(unique(...))` which is **locale-aware**. For Chinese
  # attribute names the two orderings disagree (e.g. 視 vs 防 vs 配 order
  # differs), making `arrange(attribute)` and `levels(attribute)` inconsistent.
  # That inconsistency is the upstream half of the #788 plotly zig-zag bug:
  # the line trace's row order doesn't match the (factor-level-driven) x-axis
  # order, so the rendered line jumps across categories. Casting to factor
  # first means arrange sorts by levels (dplyr's factor-aware semantics),
  # restoring the invariant `row order ≡ factor level order` per product_id.
  # Pinned by test_transform_position_to_dna_format.R Case 2 + Case 7.
  dna_data <- data %>%
    tidyr::pivot_longer(
  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      if (is.null(data) || nrow(data) == 0) {
        # #600: context-aware empty hint (mirror #595 Phase D pattern)
        prod_line_check <- product_line_id()
        no_brands_label <- if (is.null(prod_line_check) ||
                               is.na(prod_line_check) ||
                               prod_line_check == "all") {
          translate("Select product line first")
        } else {
          translate("No brands available")
        }
        updateSelectizeInput(session, "selected_brands",
                          choices = setNames("", no_brands_label),
    ...
    output$dna_plot <- renderPlotly({
      ...
      if (is.null(data) || nrow(data) == 0) {
        # #600: context-aware empty title (mirror #595 Phase D pattern)
        prod_line_check <- product_line_id()
        empty_title <- if (is.null(prod_line_check) || is.na(prod_line_check) ||
                           prod_line_check == "all") {
          translate("Please select a specific product line to view brand position analysis")
        } else {
          translate("No data available for visualization")
        }
        return(plotly_empty() %>% layout(title = empty_title))
      }
      ...
      # Refs #688: when user has set explicit brand/attribute filters, default
      # to visible traces. Pure `legendonly` default left the plot empty even
      # when the user had clearly indicated which subset they wanted to see —
      # screenshot showed correctly-rendered axes + ESS legend chip but no
      # traces, leaving users confused. With explicit filters, defaulting to
      # visible makes intent → result direct;`show_all_initially` checkbox
      # still works for the broad-default case (no filters set).
      has_explicit_filter <- length(input$selected_brands) > 0 ||
                             length(input$selected_attributes) > 0
      ...
      for (i in seq_along(product_id_groups)) {
        ...
        # #788 fix (defense in depth): re-sort this trace's rows by the
        # attribute factor so its line order matches the canonical x-axis
        # category order. The upstream `transform_position_to_dna_format`
        # fix already guarantees this invariant, but the per-trace arrange
        # is a cheap safety net against future regressions to data prep.
        # `arrange` on a factor sorts by levels (dplyr factor-aware), so
        # this is correct even if upstream regresses back to character sort.
        product_id_data <- product_id_data %>% dplyr::arrange(attribute)

        # Determine visibility based on settings.
        # `isTRUE(input$show_all_initially)` defensive guard: Shiny doesn't
        # guarantee `input$*` is bound when output$dna_plot first fires; the
        # checkbox UI may not have round-tripped from browser yet, leaving
        # input$show_all_initially as NULL. Bare `NULL || TRUE` throws
        # `Error in ||: invalid 'x' type` (regression introduced by #690).
        # isTRUE() returns FALSE for NULL/NA/non-logical, TRUE only for scalar
        # logical TRUE — exactly the conservative default-hidden semantic. (#697)
        visibility <- if (isTRUE(input$show_all_initially) || has_explicit_filter) TRUE else "legendonly"
        
        # Refs #684 Symptom A: was `x = ~levels(data$attribute)` which
        # (a) references outer `data` (full 51-row / 245-level dataset)
        # instead of the per-brand `product_id_data` iteration subset,
        # and (b) levels(factor) returns the full unique-level vector
        # (length 245) regardless of how many rows the trace contains.
        # plotly tried to recycle 245 x-values against 2-3 y-values →
        # recycle_columns "Size 2/3 vs Size 245" error + cascading
        # "discrete & non-discrete on same axis" warnings (Symptom B).
        # `~attribute` resolves to product_id_data$attribute (per-row,
        # categorical factor) — plotly handles natively as x-axis.
        p <- add_trace(
          p,
      ...
      # Refs #689: axis titles were hardcoded English. Wrap with translate()
      # so they pick up ui_terminology.csv zh_TW entries (Attribute → 屬性,
      # Score → 分數). Layout titles, like sidebar labels, are user-facing
      # strings and must go through the translate() boundary per DEV_R052.
      #
      # Refs #788: pin x-axis category order to canonical factor levels.
      # Without this, plotly defaults to `categoryorder = 'trace'` (first-
      # appearance order across traces), which can disagree with the
      # factor-level order whenever brands have sparse / different
      # attribute coverage — line traces then connect points "out of x-axis
      # sequence" and visually zig-zag. `categoryarray = fac_levels`
      # (captured at L428 from `levels(data$attribute)`) pins the axis to
      # the canonical order regardless of trace composition. Empty
      # `fac_levels` (all-NA edge case from #693) falls back to plotly
      # default — acceptable since no data means no visible mis-order.
      p <- layout(
        p,
        xaxis = list(
          title = list(
            text = translate("Attribute"),
            font = list(size = 20)
          ),
          categoryorder = if (length(fac_levels) > 0L) "array" else NULL,
          categoryarray = if (length(fac_levels) > 0L) fac_levels else NULL,
          tickangle = -45
        ),
        yaxis = list(
