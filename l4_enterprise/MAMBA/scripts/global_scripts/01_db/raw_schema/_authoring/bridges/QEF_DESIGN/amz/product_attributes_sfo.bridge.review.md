# Bridge ensemble review: QEF_DESIGN/amz/product_attributes_sfo.bridge.yaml

**Bridge yaml**: `01_db/raw_schema/_authoring/bridges/QEF_DESIGN/amz/product_attributes_sfo.bridge.yaml`
...
sfo bridge: 84 source columns, 48 rows. Part of 7-bridge batch part 2 — class-level findings inherited from bys/rpl/blb iter-1 ensemble (2026-05-05T13:00:00Z), per-PL spot-check applied here.

## Pre-flight refingerprint (this session)

Fingerprint MATCH against current CSV: `3672c46b13284490...` (canonical `infer_column_types_for_fingerprint` helper, no drift since 2026-04-29 generator authoring).

## MP160 enumeration check

source_cols (84) == column_mapping.from_column ∪ ignored_columns: PASS

## Per-PL spot-check (this session)

**Mojibake L-3** detected in cols [商品名稱]: `NoCry Safety Glasses Over Eyeglasses for Men and Women with Anti Scratch Lenses and Integrated Side Shields â Adjustable Temples with Anti-Slip Tips â 100% UV Protective Eyewear â ANSI Z87 Certified` — UTF-8/CP1252 transcoding artifact, same class as rpl L-1.
...
| S-1 | SUGGESTION | DA cross-bridge | sfo structurally consistent with blb pattern (no divergent value_map) |

## Cross-bridge note (DA, inherited)

sfo passes structural consistency checks against blb (already CONVERGED at iter 2) and bys/rpl (CONVERGED at iter 1):
