...
fn_get_position_complete_case <- function(
  ...
  if (apply_type_filter && length(type_values) > 0) {
    ...
    tryCatch({
      ...
      if (is.null(property_tbl)) {
        ...
        # Refs #688: schema drift — column is `property_name`, not `attributes`.
        # Tidyselect failed to find `attributes` as a column and fell back to
        # evaluating it as a symbol → base::attributes() primitive function →
        # "Can't extract column with !!enquo(var). must be numeric or character,
        # not a primitive function" error. Fix: use the actual column name.
      ...
      # Refs #681: include error class so empty-message errors (e.g. `!NULL`
      # which sets e$message='invalid argument type' but propagates differently
      # through the call chain) are diagnosable in the log.
      msg <- conditionMessage(e)
      if (!nzchar(msg)) msg <- "(empty message)"
      warning("Error applying type filter: ", msg,
  ...
  # Handle special rows
  # Importance added per #563 — sister of #404 F2 fix; product_profile_<pl> Gsheet
  # may carry an Importance row (per #404 D-override design) that should not leak
  # into product analysis when include_special_rows=FALSE.
  if (!include_special_rows) {
    message("🚫 Excluding special rows: Ideal, Rating, Revenue, Importance")
    ...
    if ("product_id" %in% names(tbl)) {
      ...
      tbl <- tbl %>%
        dplyr::filter(!item_id %in% c("Ideal", "Rating", "Revenue", "Importance"))
    }
  } else {
