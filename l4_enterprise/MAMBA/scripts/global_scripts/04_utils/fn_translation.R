...
initialize_translation_system <- function(language = "en", translation_dict = NULL) {
  ...
  translate <- function(text, lang = NULL) {
    # Vectorize: dplyr::rename_with(translate, ...) and similar callers pass
    # column names as a vector. Recurse element-wise. Also covers character(0).
    # See #595 Phase E: scalar `if (text %in% names(...))` errored on vectors.
    if (length(text) != 1L) {
      return(vapply(text, translate, character(1),
    ...
    # NA passes through unchanged (downstream callers expect NA preservation).
