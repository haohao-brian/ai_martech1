...
- [ ] Rawdata setup completed by user — manual Finder/symlink/mkdir per Step 3 (skill does NOT auto-copy template; the `template/organized_rawdata/` directory is reference only)
- [ ] **KEYS.xlsx sanity check passed** (0 data rows + 7-col header — see Step 3.5; default Truncate if polluted; runs *after* rawdata is in place but *before* Step 2.5a consumes KEYS — see Step 2.5a-pre)
- [ ] Product line discovery gate completed (full Google sheet scan + coverage validation) before deciding `product_line_id`
- [ ] `df_product_line.csv` created from coding sheet / sanity-check-passed KEYS.xlsx (filled by user) / manual input
