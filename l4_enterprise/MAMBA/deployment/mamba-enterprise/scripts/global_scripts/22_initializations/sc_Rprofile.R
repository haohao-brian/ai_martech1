...
.InitEnv$autoinit <- function() {
  ...
  if (!is.null(db_config$databases)) {
    if (.InitEnv$OPERATION_MODE == "APP_MODE") {
      ...
      #   - app_data   (derived layer, transactional data)
      #   - meta_data  (DM_R054 v2.1 §8: required so fn_load_product_lines
      #                 can reach canonical source; without this the
      #                 no-fallback reader would stop() on startup)
