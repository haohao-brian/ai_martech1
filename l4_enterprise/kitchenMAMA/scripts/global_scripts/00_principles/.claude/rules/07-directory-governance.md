...
| 98 | test | 測試 |
| 99 | archived | 歸檔(`archived` past-tense triggers `archived-protection` hook) |

> **Rename note (2026-05-06, #582)**:`99_archive/` → `99_archived/`(70 entries, 15M)。past-tense participle 命名讓全域 `archived-protection` hook 自動覆蓋(規則:任何路徑含 `archived` 一律禁止 destructive ops)。歷史 changelog reports 中提到 `99_archive/` 的舊條目按 MP030 archive immutability 凍結不改。
