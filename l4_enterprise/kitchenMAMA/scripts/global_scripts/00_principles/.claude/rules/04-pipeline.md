...
### MP165 v1.3 DRV-Layer Step 2 Gate(#668)
```bash
# make run 會自動跑 verify_drv_output_shape 為 final target,
# 失敗(critical fail + strict-mode)會讓 make run 整體 exit 非零。

make verify-drv                      # 直接跑 gate(不跑上游 derivation),fast re-verify
make verify-drv DRV_GATE_MODE=strict  # 強制 strict-mode 不論日期
make verify-drv DRV_GATE_MODE=warn    # 強制 warn-mode 不論日期
```

**Calendar default**:warn-mode 直到 2026-06-13,之後 strict-mode 自動翻轉。`<company>_drv.yaml` 加 `strict_mode: true` 可 sunset 前 opt-in。Gate 驗證 3 levels:L1 表存在 / L2 行數 / L3 predictor_type 分布(選用)。Sentinel rows(MP163 `'unclassified'` / `'UNKNOWN'`)預設計入 L2/L3。詳見 MP165 v1.3 `## DRV-Layer Step 2 Propagation` section + spectra change `mp165-drv-output-shape-gate`。
