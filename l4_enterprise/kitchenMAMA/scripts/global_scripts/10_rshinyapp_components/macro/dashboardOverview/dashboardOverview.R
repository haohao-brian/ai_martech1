...
dashboardOverviewComponent <- function(id, app_connection, comp_config, translate) {
  ...
  ui_display <- tagList(
    # #795 #797 #807 #808: empty-state banner (MP163 visible-gap sentinel)
    # 當 dna_data + macro_latest 都為 NULL → 顯示 banner 說明可能原因
    uiOutput(ns("data_coverage_banner")),
  ...
  server_fn <- function(input, output, session) {
    # #603 (#588): capture parent input BEFORE moduleServer shadows the
    # `input` parameter. We need parent_input$sidebar_menu to gate reactives
    # so they only fire when this tab is active. comp_config() invalidations
    # propagate cross-tab; without this guard the 225K-row dna_data and two
    # macro reactives re-fire on every tab switch (BrandEdge / VitalSigns
    # / etc), wasting CPU + I/O. Evidence: #587 R log shows
    # `[dashboardOverview] DNA data: 225557 records` firing during
    # BrandEdge tab switch.
    parent_input <- input

    moduleServer(id, function(input, output, session) {

      # --- Tab-active guard helper (#603) ---
      # Returns TRUE when the dashboardOverview tab is active OR when the
      # nav input is not yet populated (initial load — let reactives compute
      # so first render is immediate). Returns FALSE when the user has
      # navigated to another tab.
      .is_dashboard_active <- reactive({
        v <- parent_input$sidebar_menu
        is.null(v) || length(v) == 0 || v == "dashboardOverview"
      })

      # --- Data reactives ---

      dna_data <- reactive({
        req(.is_dashboard_active())  # #603: gate cross-tab leak
        cfg <- comp_config()
      ...
      # --- Data coverage banner (#795 #797 #807 #808) ---
      # 當 DNA + macro 兩個 source 都沒資料 → 顯示 visible-gap sentinel banner
      # 不靜默 (per MP163 progressive completeness),讓客戶知道是「資料層」問題
      # 而非 dashboard bug,並引導去 cross-issue 看資料覆蓋進度
      output$data_coverage_banner <- renderUI({
        dna <- dna_data()
        macro <- macro_latest()
        if (!is.null(dna) || !is.null(macro)) return(NULL)  # 至少一個有資料 → 不顯示 banner

        # 兩個 source 都 NULL → 顯示 explicit empty-state(per MP163)
        cfg <- comp_config()
        pl_id <- cfg$filters$product_line_id_sliced %||% "all"
        ...
        div(style = paste0(
          ...
          tags$strong(icon("info-circle"), " ",
                      translate("No data for this filter combination")),
          tags$br(),
          tags$small(
            translate("Filter:"), " platform=", tags$code(plt), " / product_line=", tags$code(pl_id),
            tags$br(),
            translate("Possible reasons:"),
            tags$ul(style = "margin: 4px 0 0 20px;",
              tags$li(translate("Product line has no sales yet on this platform (e.g. sgf/psg/rpl on Amazon — known gap, see issue #774)")),
              tags$li(translate("ETL pipeline has not run for this combination yet")),
              tags$li(translate("Filter combination is genuinely empty (e.g. PL × platform mismatch)"))
            ),
            tags$br(),
            translate("Try:"), " ",
            translate("select a different product line, or check ETL run status with operations team.")
          )
        )
      })
      ...
      # 5. Repurchase rate (#320 + #801/#812 rename): repeat buyers (nt>=2) / total customers
      # Element ID preserved as `gm_conversion_rate` (避免 breaking change),
      # subtitle/label renamed to "Repurchase Rate" per #801 customerAcquisition pattern,
      # 跨 module 標籤一致(#812 cross-module 標籤不一致 fix)
      output$gm_conversion_rate <- renderUI({
        ...
        make_vbox(val, translate("Repurchase Rate"), "exchange-alt", "purple")
      })
