  ...
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      if (is.null(data) || nrow2(data) == 0) {
        # #595 Phase D: context-aware empty hint
        ...
        # Selectize choices is named char vector: name = display label, value = ""
        empty_choice <- setNames("", no_brands_label)
      ...
      if (is.null(prod_line_check) || is.na(prod_line_check) || prod_line_check == "all") {
        ...
        output$component_status <- renderText({
          # #595 Phase D: was hardcoded English, now translated via shared key.
    ...
    output$position_table <- renderDT({
      ...
      data <- position_data()
      if (is.null(data) || nrow2(data) == 0) {
        # #595 Phase D: context-aware empty state. When prod_line == "all" the
        # reactive intentionally returns empty (cross-product-line comparison
        # is not meaningful), but stock "No data available" misleads users
        # into thinking the system is broken. Surface the action they need.
        prod_line_check <- product_line_id()
      ...
      # Refs #681: isTRUE() defensive against NULL (first render, before checkbox
      # UI initialized) — `!NULL` and `if(NULL)` both error with "invalid argument
      # type" / "argument is of length zero". isTRUE() returns FALSE for both
      # NULL and non-logical, matching the intended "default = hide ideal" UX.
      if (!isTRUE(input$show_ideal)) {
      ...
      ideal_row_data <- NULL
      if (isTRUE(input$show_ideal)) {   # Refs #681 — NULL-safe
      ...
      # Refs #681: isTRUE() guards against NULL input$show_ideal on first render
      if (!is.null(ideal_row_data) && isTRUE(input$show_ideal) && nrow2(ideal_row_data) > 0) {
    ...
    output$component_status <- renderText({
      # Special message when product_line is "all" — #595 Phase D
      if (product_line_id() == "all") {
        return(translate("Please select a specific product line to view brand position analysis"))
      }
