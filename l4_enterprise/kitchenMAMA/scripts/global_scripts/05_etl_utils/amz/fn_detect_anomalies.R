...
build_mapping_gaps <- function(resolved_master, sales_pairs, sku_pl_map,
  ...
  if (any(sku_missing)) {
    ...
    #
    # #473: in main pipeline this branch is dead — A' Step 2.5 backfills asin
    # before the resolver runs, so those rows enter master and `sku %in%
    # master_skus` becomes TRUE → no_master_row never triggers. The note
    # below remains for `detect_anomalies` standalone invocations (when A'
    # has not run, e.g. ad-hoc analysis on raw sales). Production-side
    # surfacing is now handled by `df_amazon_sales_coverage_audit` write at
    # 2TR layer with reason='asin_as_sku_fallback' (MP163 §3 alignment).
  ...
  if (is.null(sku_pl_map) || nrow(sku_pl_map) == 0) {
    message(
      "[build_mapping_gaps] catalogue empty (df_amz_product_master has 0 ",
      "rows with SKU) — skipping no_product_line gap detection. Per #469 ",
      "(resolved): master now includes SKU column when product_profile ",
      "Gsheet tabs populate it. Empty SKU column means tabs lack SKU data; ",
      "populate Gsheet to enable catalogue-side gap detection."
    )
  } else {
