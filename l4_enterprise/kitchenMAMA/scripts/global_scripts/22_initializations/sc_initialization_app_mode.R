...
load_dirs <- c(
  ...
  # Cross-mode shared string utilities (make_names / fn_convert_all_columns_to_utf8).
  # BrandEdge type filter at fn_get_position_*_case.R calls make_names without a
  # defensive `if (!exists()) source(...)` workaround, so app mode crashes on
  # `could not find function "make_names"` without this entry. ETL mode loads
  # all of 05_etl_utils so it's unaffected. (#698)
  "05_etl_utils/common/string",
