...
filter_excluded_covariates <- function(data, predictor_col = "predictor", ...) {
  ...
  # Empty-df guard (#713): sapply(character(0), should_exclude_covariate, ...)
  # returns list() not logical(0); subsequent `data[!exclude_mask, ]` then evaluates
  # `!list()` and throws "invalid 'type' (list) of argument". This propagates as
  # "Error loading rating analysis data: ..." in poissonCommentAnalysis whenever
  # an upstream filter chain leaves zero rows for a product line (e.g. QEF sfg).
  # Same archetype as #655 (which guarded positive_data but missed this code path).
  if (nrow(data) == 0) {
    message("Filtered 0 of 0 covariates (empty input)")
    return(data)
  }
